...
if menu == "闪卡记忆":
    ...
    st.markdown(
        "✨ 闪卡记忆是一种记忆单词的游戏，其玩法是将单词或短语的中英文对照显示在屏幕上，玩家需要根据提示信息，尽可能多地记住单词或短语的含义。这种游戏可以帮助玩家记忆单词的含义。用户可以在侧边栏选择发音标准、单词列表和计划记忆的单词数量。在主页面上，用户可以通过点击按钮来切换显示状态、切换到上一个或下一个单词、播放单词发音、将当前单词添加到或从个人词库中删除。如果用户在侧边栏改变了词库或记忆数量，他们可以点击一个按钮来重新生成闪卡单词。"
    )
